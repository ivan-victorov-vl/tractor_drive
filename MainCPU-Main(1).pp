...
void CellInitDoneCheck();
void CellWorkCheck();
void sel_SPI_dev(Uint16 dev);
...
void main(void)
{
	...
	InitEPwm_4_6Timers(5552 * 1.5, 700 * 1.5, 1200 * 1.5);
	...
	ConfigCpuTimer(&CpuTimer2Regs, Prd2, 1);		
	...
	all_init_id = init_off;									
	CellInitDoneCheck();


	for(;;)
		{
		if(tx.serv.bit.mode == 0) 
			{
			CellInitDoneCheck();
			}
		else
...
void CellWorkCheck()
	...
	if((!tot_cell_ready) & (tot_sgnl_err))
		{
		tx.serv.bit.mode = 0;
		all_init_id = init_off;		
		GpioDataRegs . GPBCLEAR . bit . GPIO49 = 1;
...
Uint16	n = &cell[0].word.cell_indx - &cell[0].all[0];	
	...
	tx.Smpl_F[0].all = tx.Smpl_F[1].all = tx.Smpl_F[2].all = 41640/2; 

	
	{		
	Uint16* p = &store[0].serv.all;
	Uint16 i;
	for(i=0;i<8*2;i++) *(p + i) = 0;
	}
	...
	interrupt void epwm4_timer_isr(void)
		...
		if(store[old].serv.bit.mode)				
			...
			if(numb_frames != 5)					
				...
				cell[store[old].data[1] * 5 + store[old].data[0]].word.sgnl_err = 1;
		...
		ScibRegs.SCICTL1.bit.RXENA	=	1;			
			...
			sel_SPI_dev(0x8);
			for(;SpiaRegs.SPIFFRX.bit.RXFFST >= 1;)		
				{
				SPI_RX_frame = SpiaRegs.SPIRXBUF; 		
				GpioDataRegs . GPBTOGGLE . bit . GPIO39 = 1;
				}
			Exp1_regs . GPout . bit . GPIO16 ^= 1; 							
			SpiaRegs.SPITXBUF = 0x0000 + 0x4C00 + Exp1_regs.GPout.byte.GP12_19;	
	...
	static	Uint16 frame_cnt = 0;		
		...
		if(tx.serv.bit.mode)		
			...
			if(tx.serv.bit.cell_indx >= 5) tx.serv.bit.cell_indx = 0;
			...
			static	Uint16	init_flag = 0;		

			if((!all_init_id) & (!init_flag))	
				param_cnt = i_cell_cnt = 0;		
			...
			init_flag = !all_init_id;
			store[old].all_init = all_init_id;
		...
		{
		Uint16 a;
		store[old].serv.all = tx_frame[0];
		for(a=0;a < 6;a++)
			store[old].data[a] = tx_frame[a+1];
		old ^= 1;									
		}
	...
	if(ScibRegs.SCIFFRX.bit.RXFFINT)				
		...
		if(store[old].serv.bit.mode)			
			...
			if(numb_frames == 9)		
				...
				for(i=0;i<3;i++)
					{
					n = ((new_datain[i*3] & 0x30) / 0x10) * 5 + (new_datain[i*3] & 0xF);
			...
			if(numb_frames == 5)		
				...
				if(new_datain[0] & 0x80)		
					...
					Nc_resp =  new_datain[4] * 5 + new_datain[3]; 		
					Nc_req = store[old].data[1] * 5 + store[old].data[0];	
					Np_resp =  new_datain[0] & 0x3F;							
					Np_req = store[old].data[2];								
					...
					if((Nc_resp != Nc_req)|(Np_resp != Np_req))	
						...
						if(Np_resp == nmb_of_askprm - 1) 
							...
							if(p_cnt == nmb_of_askprm)	   
								...
								if(store[old].all_init)  all_init_id = RX_init_done;
...
interrupt void cpu_timer2_isr(void)
{
	...
	void convert_ind(Uint16 value, int16* ind_str) 
	{
	Uint16 flag = 0, i, sign = 0x20, divider = 10000, dig;
		...
		for(i=1;i<5;i++)
			...
			if(flag)
				...
				*(ind_str+i) = dig + 0x30;
				...
				else *(ind_str+i-1) = 0x20;
				}
			value = value - (dig*divider);
			divider /= 10;
			}
